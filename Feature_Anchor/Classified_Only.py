...
def run_linearsvr_cv(data_path, target="ACE-km", mode="ace_only"):
    ...
    ----------
    data_path : str
        Path to the classified data file
    target : str
    ...
    # Load the feature lists from files
    ...
    if mode == "h2":
        ...
        selected_features = list(set(acetoclastic + hydrogenotrophic + syntrophic))  # Remove any duplicates
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        if mode == "ace_conditional":
            ...
            if sum(mask_train_high) > 0:
                # Use all classified features for high ACE-km
                X_train_high = X_train[mask_train_high]
                scaler_high = StandardScaler()
                X_train_high_scaled = scaler_high.fit_transform(X_train_high)
                
                model_high = LinearSVR(random_state=42, max_iter=100000, tol=1e-4, dual=True)
